...
$(document).ready(function() {
    // 本地存储键名
    const STORAGE_KEY = 'dark_mode_status';
    ...
    const applyDarkMode = (isDark) => {
        // 1. 更新Body类
        $('body').toggleClass('io-black-mode', isDark);
        $('body').toggleClass(theme.defaultclass, !isDark);
    ...
    $(document).on('click', '.switch-dark-mode', function(event) {
        ...
        const isCurrentDark = $('body').hasClass('io-black-mode');
        ...
        // 保存到本地存储
        localStorage.setItem(STORAGE_KEY, newMode ? 'on' : 'off');
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                mode_toggle: newMode ? 1 : 0,
                action: 'switch_dark_mode',
            }
        }).fail(function() {
            console.error('服务器同步失败，但本地已切换');
            // 无需回滚，因为本地状态优先
        });
    });
...
function switch_mode(isDarkMode) {
    const $button = $('.switch-dark-mode');
    
    // 更新提示文本
    const tooltipText = isDarkMode ? '日间模式' : '夜间模式';
    $button.attr({
        'title': tooltipText,
        'data-original-title': tooltipText
    });
    
    // 更新图标（确保逻辑正确：深色模式对应夜间图标）
    const $icon = $button.find('.mode-ico');
    $icon.toggleClass('icon-night', isDarkMode);
    $icon.toggleClass('icon-light', !isDarkMode);
}
    //返回顶部
    document.addEventListener('DOMContentLoaded', function() {
        const goUpBtn = document.getElementById('go-to-up');
        const header = document.querySelector('.big-header-banner');
        let scrollTimer = null;
        
        // 获取按钮原始样式
        const originalBtnStyles = window.getComputedStyle(goUpBtn);
        
        // 创建图标容器
        const iconContainer = document.createElement('span');
        iconContainer.className = 'go-up-icon-container';
        iconContainer.style.cssText = `
            display: block;
            width: 100%;
            height: 100%;
            text-align: center;
            line-height: ${originalBtnStyles.height};
        `;
        
        // 将原有图标移入容器
        const icon = goUpBtn.querySelector('i');
        iconContainer.appendChild(icon);
        
        // 创建百分比容器
        const percentageContainer = document.createElement('span');
        percentageContainer.className = 'go-up-percentage-container';
        ...
        percentageEl.className = 'go-up-percentage';
        percentageContainer.appendChild(percentageEl);
        
        // 清空按钮并添加两个容器
        goUpBtn.innerHTML = '';
        goUpBtn.appendChild(iconContainer);
        goUpBtn.appendChild(percentageContainer);
        
        // 设置按钮为相对定位
        if (originalBtnStyles.position === 'static') {
            goUpBtn.style.position = 'relative';
        }
        ...
        function handleScroll() {
            ...
            // 设置新定时器，2秒后恢复显示图标
            scrollTimer = setTimeout(showIcon, 2000);
        }
